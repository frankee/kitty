...
func main() {
	...
	// and finally, a series of concrete grpc_transport adapters. The adapters, like
	...
	var g group.Group
	{
		...
		if err != nil {
			logger.Log("grpc_transport", "debug/HTTP", "during", "Listen", "err", err)
			os.Exit(1)
		}
		g.Add(func() error {
			logger.Log("grpc_transport", "debug/HTTP", "addr", *debugAddr)
	...
	}
	{
		...
		if err != nil {
			logger.Log("grpc_transport", "HTTP", "during", "Listen", "err", err)
			os.Exit(1)
		}
		g.Add(func() error {
			logger.Log("grpc_transport", "HTTP", "addr", *httpAddr)
	...
	}
	{
		...
		if err != nil {
			logger.Log("grpc_transport", "gRPC", "during", "Listen", "err", err)
			os.Exit(1)
		}
		g.Add(func() error {
			logger.Log("grpc_transport", "gRPC", "addr", *grpcAddr)
